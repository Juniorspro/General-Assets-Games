...
(function () {
  ...
  fetch("/api/sitio", { headers: { Accept: "application/json" } })
    ...
    .then(function (d) {
      ...
      /* Sin el caso vacío esto no se cura solo: si en el servidor se borra la
         lista, el visitante que ya tenía la vieja en su copia local la sigue
         dibujando para siempre, porque `ahora` queda en blanco y la condición
         no entra. Pasó de verdad. */
      if (ahora !== antes) {
        try {
