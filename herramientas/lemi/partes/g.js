...
const GUION = [
  [  6.0, 'g0' ],   /* llegan */
  [ 13.0, 'g1' ],   /* arman todo */
  [ 19.0, 'g2' ],   /* el campamento quedó armado */
  [ 24.0, 'g3' ],   /* el ruido */
  [ 29.5, 'g4' ],   /* se van a dormir */
  [ 34.5, 'g5' ],   /* se despertó solo */
  [ 38.0, 'g6' ]    /* el rastro */
];
const INTRO = {
  /* LOS TIEMPOS DE LA HISTORIA. Cada nombre es el segundo en que EMPIEZA su
  ...
  termina(){
    ...
    CFG.sol = 0.28;
    FUEGO_ON = true;
    if (fuegoMalla) fuegoMalla.visible = true;
    for (const k of CARPAS) if (k.malla){ k.malla.visible = true; k.malla.scale.set(1,1,1); }
    entraJuego();
  ...
  pon(t){
    ...
    /* LOS TIEMPOS DE LA HISTORIA, EN UN SOLO LUGAR. Estaban escritos como
       números sueltos repartidos por toda la función —el sol miraba 12, el
       fuego miraba 9 y 22,5, la noche miraba 12 y 22,5— y agregar una escena al
       principio obligaba a encontrarlos todos y correrlos uno por uno. Con la
       tabla, correr una escena es cambiar un número. */
    const T0 = INTRO.T;
    /* la tarde cae, la noche pasa, y de la noche se corta a la mañana. El corte
       cae DENTRO del negro, que es lo que hace que se lea como que pasó la
       noche y no como que el sol pegó un salto. */
    CFG.sol = t < T0.sienta ? 0.680 + t*0.0032
            : t < T0.duerme + 3.5 ? 0.742 + (t-T0.sienta)*0.0106
            : t < T0.corte ? 0.871
            :               0.245 + cl((t-T0.corte)/5, 0, 1)*0.055;
    ponSol(0);
    if (this.foco && !(t >= T0.ruido && t < T0.duerme)) this.foco.intensity = 0;
    const deNoche = t > T0.prende && t < T0.corte;
    if (fuegoLuz) fuegoLuz.distance = deNoche ? 40 : 26;
    FUEGO_K = deNoche ? 3.0 : 1;
    /* LA FOGATA NO ESTÁ PRENDIDA DESDE EL PRIMER CUADRO: la prenden ellos, y
       hasta entonces son piedras y leña apagada. Es la mitad de la escena nueva
       —«armaron todo»— y sin esto el fuego ya estaba ahí antes de que llegaran. */
    FUEGO_ON = t >= T0.prende;
    if (fuegoMalla) fuegoMalla.visible = FUEGO_ON;
    /* y las carpas se levantan una por una mientras las arman */
    for (let i = 0; i < CARPAS.length; i++){
      const k = CARPAS[i];
      if (!k.malla) continue;
      const cuando = T0.arma + 1.2 + i*1.5;
      const sale = cl((t - cuando)/1.1, 0, 1);
      k.malla.visible = sale > 0.02;
      /* se levanta creciendo desde el piso, que es lo que hace una carpa de
         varillas al tensarse: sube en Y y casi no cambia de planta */
      k.malla.scale.set(0.55 + sale*0.45, sale*sale, 0.55 + sale*0.45);
    }
    /* la noche se levanta sólo mientras se la mira de cerca, igual que antes */
    if (t > T0.ruido && t < T0.corte){
      const lift = cl((t - T0.ruido)/2.5, 0, 1);
    ...
    if (t < T0.arma){
      /* ── 1 · LLEGAN AL CLARO ──
         Entran caminando desde donde está la camioneta. El auto ya está
         estacionado —eso no cambió— y lo que se ve es a los cuatro cruzando el
         claro con el sitio todavía vacío: sin carpas, sin fuego. Que el lugar
         esté PELADO es la escena: es lo que hace que la siguiente, en la que
         aparece todo, se lea como que lo armaron ellos. */
      const k = cl(t/T0.arma, 0, 1);
      /* de dónde vienen: del auto si existe, y si no de un costado */
      const dx = a ? a.x - c.x : 12, dz = a ? a.z - c.z : 12;
      const dl = Math.hypot(dx, dz) || 1;
      const ex = dx/dl, ez = dz/dl;
      g.forEach((p, i) => {
        p.visible = true;
        p.userData.leno.visible = false;
        /* en fila despareja, que cuatro personas caminando alineadas se leen a
           desfile: cada uno con su distancia y su corrimiento de costado */
        const atras = 1.0 + i*1.15, lado = ((i%2) ? 1 : -1) * (0.8 + (i>>1)*0.7);
      ...
      /* la cámara los espera en el claro, a la altura de la vista, y se corre
         ...
         hay para decir «llegaron» */
      /* LA CÁMARA VA AL COSTADO DEL CAMINO POR EL QUE VIENEN, no encima. Puesta
         sobre esa línea, a nueve metros del centro en la dirección del auto,
         cayó DENTRO de la camioneta —que está justo ahí— y el plano entero
         salió rojo. Corrida sobre la perpendicular, los ve cruzar de lado, que
         además es como se lee mejor un grupo caminando. */
      /* Y MIRA DONDE ESTÁN, no un punto fijo. La primera versión apuntaba a
         cinco metros del centro y el grupo entraba por un costado del cuadro:
         el plano decía «un claro vacío» y los cuatro pasaban de refilón. El
         punto de mira sale de la MISMA cuenta que los coloca —`d0` con el
         atraso del medio— así que los sigue por construcción. */
      const px2 = -ez, pz2 = ex;
      const dGrupo = 13.5 - k*10.0 + 2.7;
      const mx = c.x + ex*dGrupo, mz = c.z + ez*dGrupo;
      const cx = mx + px2*9.5 + ex*1.5;
      const cz = mz + pz2*9.5 + ez*1.5;
      cam.position.set(cx, c.h + 2.3, cz);
      cam.lookAt(mx, H(mx, mz) + 1.05, mz);
      cam.fov = 54;
    } else if (t < T0.sienta){
      /* ── 2 · ARMAN TODO ──
         Dos levantan las carpas —que van saliendo del piso una por una, con el
         reloj de arriba— y dos ponen los troncos en la fogata hasta prenderla.
         Cada uno hace UNA cosa y se queda en su sitio: cuatro personas yendo y
         viniendo por el claro se leen a caos, no a trabajo. */
      const u = t - T0.arma, k = cl(u/(T0.sienta - T0.arma), 0, 1);
      g.forEach((p, i) => {
        p.visible = true;
        if (i < 2){
          /* los dos de la fogata: agachados poniendo leña, con el tronco en la
             mano hasta que el fuego prende */
          const s2 = sitio(i);
          const fx = lerp(s2.x, c.x, 0.62), fz = lerp(s2.z, c.z, 0.62);
          p.position.set(fx, H(fx, fz), fz);
          p.rotation.y = Math.atan2(c.x - fx, c.z - fz);
          p.userData.leno.visible = t < T0.prende;
          poseLena(p, RELOJ.value + i*0.8, t < T0.prende);
        } else {
          /* los dos de las carpas: parados al lado de la suya, trabajando */
          const ca = CARPAS.length ? CARPAS[(i-2) % CARPAS.length] : sitio(i);
          const hx = lerp(c.x, ca.x, 0.82), hz = lerp(c.z, ca.z, 0.82);
          p.position.set(hx, H(hx, hz), hz);
          p.rotation.y = Math.atan2(ca.x - hx, ca.z - hz);
          p.userData.leno.visible = false;
          poseLena(p, RELOJ.value + i*1.1, true);
        }
      });
      /* la cámara rodea el claro entero: hay que ver las carpas Y el fuego, y
         están en lados distintos */
      const ang = 1.15 + k*1.05, rr = 12.4 - k*2.6;
      const cx = c.x + Math.cos(ang)*rr, cz = c.z + Math.sin(ang)*rr;
      cam.position.set(cx, c.h + 3.6 - k*0.9, cz);
      cam.lookAt(c.x, c.h + 1.25, c.z);
      cam.fov = 55 - k*4;
    } else if (t < T0.ruido){
      /* ── 3 · LOS CUATRO EN LA FOGATA, CAYENDO EL SOL ──
         Sentados y charlando. La cámara los rodea despacio y baja: es el plano
         que establece que son cuatro y que están bien. */
      const k = cl((t - T0.sienta)/(T0.ruido - T0.sienta), 0, 1);
      g.forEach((p, i) => {
        p.visible = true;
        p.userData.leno.visible = false;
        const s2 = sitio(i);
      ...
      const u = t - T0.ruido, k = cl(u/(T0.duerme - T0.ruido), 0, 1);
      ...
      const u = t - T0.duerme, k = cl(u/4.2, 0, 1);
      g.forEach((p, i) => {
      ...
      const u = t - T0.corte, k = cl(u/8.5, 0, 1);
    ...
    $('cVelo').classList.toggle('ver',
      (t > T0.corte - 1.1 && t < T0.corte + 0.9) || t > this.dur - 1.1);
  },
  paso(dt){
    this.t += dt;
    this.pon(this.t);
