...
const srv=http.createServer((q,r)=>{ let u=decodeURIComponent(q.url.split('?')[0]); if(u==='/')u='/eco.html';
 const f=path.join('/tmp/ui',u);
 try{ if(!fs.existsSync(f)||fs.statSync(f).isDirectory()){ r.writeHead(404); r.end('no'); return; } }catch(e){ r.writeHead(404); r.end('no'); return; }
...
await pg.goto('http://127.0.0.1:8098/eco.html?v='+Date.now(),{waitUntil:'domcontentloaded'});
