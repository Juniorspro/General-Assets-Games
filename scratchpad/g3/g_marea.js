...
async function init3d(THREE) {
  ...
  water = new T.Mesh(new T.PlaneGeometry(1300, 1300, 1, 1), new T.MeshStandardMaterial({ color: 0x37cfe6, transparent: true, opacity: .5, roughness: .12, metalness: .05, envMap: sky, envMapIntensity: .55, bumpMap: bump, bumpScale: .16, depthWrite: false }));
  ...
  const laneMat = new T.MeshStandardMaterial({ color: 0xffffff, roughness: .5, emissive: 0x88bbcc, emissiveIntensity: .25 });
  for (let i = 0; i < NG * 2; i++) { const bu = new T.Mesh(new T.CylinderGeometry(.42, .52, 1.5, 8), laneMat); scene.add(bu); LANE.push({ m: bu, x: 0, z: 0 }); }
...
function layoutTrack() {          // ubica boyas de puerta + canal según GATES
  ...
  for (let i = 0; i < NG; i++) { const a = GATES[i], b = GATES[(i + 1) % NG]; const mx = (a.x + b.x) / 2, mz = (a.z + b.z) / 2;
    const dx = b.x - a.x, dz = b.z - a.z, dl = Math.hypot(dx, dz) || 1, px = -dz / dl, pz = dx / dl;
    [-1, 1].forEach((sgn, k) => { const L = LANE[i * 2 + k]; L.x = mx + px * sgn * 8; L.z = mz + pz * sgn * 8; L.m.position.set(L.x, .3, L.z); }); }
}
...
function step(dt) {
  ...
  for (const l of LANE) l.m.position.y = .3 + waveH(l.x, l.z, t);
