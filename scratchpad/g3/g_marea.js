/* ===== MAREA — moto de agua (laguna tropical cristalina) ===================
   Agua tipo pileta: CRISTALINA, se ve la ARENA del fondo con CÁUSTICAS que
   titilan, olitas chiquitas y muchas, reflejo del cielo y SOMBRAS en tiempo
   real. La moto flota con físicas. Circuito de boyas · 3 modos. */
...
/* olitas: muchas y chiquitas (agua de pileta, casi plana) */
...
function buildCircuit() { GATES = [];
  for (let i = 0; i < NG; i++) { const a = i / NG * 6.283; const r = 155 * (.82 + .26 * Math.sin(a * 3 + .6)); GATES.push({ x: Math.cos(a) * r, z: Math.sin(a) * r }); } }
...
async function init3d(THREE) {
  ...
  scene.add(new T.HemisphereLight(0xffffff, 0x3aa0c0, 1.2));
  scene.add(new T.AmbientLight(0xffffff, .3));
  const sun = new T.DirectionalLight(0xfff6e6, 2.6); sun.position.set(-50, 95, -35);
  sun.castShadow = true; sun.shadow.mapSize.set(1024, 1024);
  const sc = sun.shadow.camera; sc.left = -130; sc.right = 130; sc.top = 130; sc.bottom = -130; sc.near = 1; sc.far = 220; sun.shadow.bias = -0.0006;
  scene.add(sun);
  ...
  // FONDO de arena (se ve a través del agua) — recibe sombras
  const sand = new T.Mesh(new T.PlaneGeometry(1300, 1300), new T.MeshStandardMaterial({ map: rep(TEX.sand, 34), color: 0xcdeef2, roughness: 1 }));
  sand.rotation.x = -Math.PI / 2; sand.position.y = -4; sand.receiveShadow = true; scene.add(sand);
  // CÁUSTICAS: dos capas aditivas que se desplazan → titilan sobre la arena
  ...
  caus1 = new T.Mesh(new T.PlaneGeometry(1300, 1300), cauMat(16, .55)); caus1.rotation.x = -Math.PI / 2; caus1.position.y = -3.7; scene.add(caus1);
  caus2 = new T.Mesh(new T.PlaneGeometry(1300, 1300), cauMat(11, .4)); caus2.rotation.x = -Math.PI / 2; caus2.position.y = -3.55; scene.add(caus2);
  // AGUA cristalina (transparente, refleja el cielo, ripple por bumpMap animado)
  const bump = rep(TEX.caustics, 60);
  water = new T.Mesh(new T.PlaneGeometry(1300, 1300, 1, 1), new T.MeshStandardMaterial({
    color: 0x37cfe6, transparent: true, opacity: .5, roughness: .12, metalness: .05,
    envMap: sky, envMapIntensity: .55, bumpMap: bump, bumpScale: .16, depthWrite: false }));
  ...
  if (!m) { m = new T.Group();
    const hull = new T.Mesh(new T.BoxGeometry(1.5, .7, 3.6), new T.MeshStandardMaterial({ color: 0xd83a3a, roughness: .5, metalness: .2 })); hull.geometry.translate(0, .35, 0); hull.castShadow = true; m.add(hull);
  ...
  const buoy = (col, dx) => { const gg = new T.Group();
    const body = new T.Mesh(new T.CylinderGeometry(.9, 1.1, 2.2, 10), new T.MeshStandardMaterial({ color: col, roughness: .5, emissive: col, emissiveIntensity: .4 })); body.position.y = 1; body.castShadow = true; gg.add(body);
  ...
  // ISLAS GRANDES (GLB) con sombras + gente + pájaros
  LIFE.setup(T);
  ...
  let islTpl = null; try { const ig = await ARC.loadGLB(MDL.island); islTpl = ig.scene; islTpl.traverse(o => { if (o.isMesh) { o.castShadow = true; o.receiveShadow = true; if (o.material) o.material.metalness = 0; } }); } catch (e) {}
...
function endWin() { won = true; dead = true; const bonus = Math.round(timeLeft * 6); ARC.over({ win: true, score: score + bonus, title: '¡CAMPEÓN!', sub: laps + ' vueltas · +' + bonus + ' por tiempo', coins: ((score + bonus) / 30 | 0) }); }
...
function step(dt) {
  ...
  // agua: cáusticas + ripple se desplazan (titilan) — sin tocar vértices (barato)
  const t = tPlay;
  caus1.material.map.offset.set(t * .012, t * .009); caus2.material.map.offset.set(-t * .009, t * .013);
  water.material.bumpMap.offset.set(t * .02, t * .016);
  ...
  // FÍSICA en el agua: flota (resorte), cabecea/rola con las olitas
