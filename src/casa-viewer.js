...
function buildHouse() {
  ...
  // tejado a dos aguas
  const rh = 3.0, slab = Math.hypot(W / 2 + 0.5, rh), ang = Math.atan2(rh, W / 2 + 0.5);
  for (const s of [1, -1]) { const r = add(new THREE.BoxGeometry(slab, 0.22, D + 1.2), roofMat, s * (W / 4), 0.8 + H + rh / 2, 0); r.rotation.z = -s * ang; }
  // frontones (triángulos) tapados con tablas
  for (const sz of [1, -1]) {
    const tri = new THREE.Mesh(new THREE.BufferGeometry(), wallMat);
    const hw = W / 2; const verts = new Float32Array([-hw, 0, 0, hw, 0, 0, 0, rh, 0]);
    tri.geometry.setAttribute('position', new THREE.BufferAttribute(verts, 3)); tri.geometry.computeVertexNormals();
    tri.position.set(0, 0.8 + H, sz * D / 2); tri.castShadow = true; g.add(tri);
  }
