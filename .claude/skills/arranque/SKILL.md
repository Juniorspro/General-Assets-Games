...
Un plan es una lista de `{"js": …}`, `{"click": …}`, `{"tap": [x,y]}`, `{"key": "KeyW", "ms": …}`,
`{"wait": ms}`, `{"n": "nombre"}` (captura). Las capturas salen a `/tmp/ui/out/` en **412×892
vertical y derechas** — el banco de `herramientas/banco/` ya no las gira, así que **no** hay que
pasarles `Image.rotate`.

`MOVIL=1` enciende el joystick y los botones táctiles: el CSS los muestra con
`@media (hover: none) and (pointer: coarse)`, que sin `isMobile` no da.

> **Una tecla no se lee cada cuadro, se engancha.** Headless con SwiftShader corre a **uno o dos
> cuadros por segundo**: un toque corto cae entero entre dos cuadros y no existe. Por eso el paso
> `{"key": …, "ms": …}` mantiene la tecla, y por eso el juego late los controles en el `keydown`.
> Del mismo lado: **una sonda leída en el mismo instante que la acción devuelve el valor viejo** —
> hay que dejar pasar un `wait` de verdad antes de creerle.
