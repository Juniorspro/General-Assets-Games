    ...
    loadPhone() {
        ...
        loader.load(CDLV_PHONE, gltf => {
            ...
            // el eje mas corto del modelo es el grosor: de ahi salen las dos caras planas
            ph.updateMatrixWorld(!0);
            let pb = new fP().setFromObject(ph), ps = pb.getSize(new Y), pc = pb.getCenter(new Y);
            let dims = [{ a: "x", v: ps.x }, { a: "y", v: ps.y }, { a: "z", v: ps.z }].sort((p, q) => p.v - q.v);
            let thin = dims[0].a, half = dims[0].v / 2 + .0025;
            let holder = new eP;
            holder.add(ph);
            ...
            for (let s of [1, -1]) {
                let w, h, g;
                if (thin === "y") w = ps.x * .8, h = ps.z * .84;
                else if (thin === "z") w = ps.x * .8, h = ps.y * .84;
                else w = ps.z * .84, h = ps.y * .8;
                g = new fA(new k9(w, h), glowMat);
                if (thin === "y") g.position.set(pc.x, pc.y + s * half, pc.z), g.rotation.x = -s * Math.PI / 2;
                else if (thin === "z") g.position.set(pc.x, pc.y, pc.z + s * half), s < 0 && (g.rotation.y = Math.PI);
                else g.position.set(pc.x + s * half, pc.y, pc.z), g.rotation.y = s * Math.PI / 2;
                holder.add(g); glows.push(g);
            }
            let light = new GD(0x9fd4ff, 1.1, 1.6, 2);
            light.position.set(pc.x, pc.y, pc.z); holder.add(light);
