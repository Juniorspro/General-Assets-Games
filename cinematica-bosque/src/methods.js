    ...
    loadPhone() {
        ...
        loader.load(CDLV_PHONE, gltf => {
            ...
            let holder = new eP;
            holder.add(ph);
            holder.position.set(0, -.05, .03);
            holder.rotation.set(-1.78, 0, .12);
            holder.visible = !1;
            // el modelo ya trae la pantalla texturizada: solo se le suma la luz que tira
            let light = new GD(0x9fd4ff, 1.4, 1.8, 2);
            light.position.set(0, .05, 0); holder.add(light);
            this.pov.armR.userData.hand.add(holder);
            this.pov.phone = ph; this.pov.phoneHolder = holder; this.pov.phoneLight = light;
        }, void 0, () => { });
    }
