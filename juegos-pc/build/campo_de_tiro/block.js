...
function tiroTracer(){ for(const t of tiroTracers) if(t.userData.v<=0) return t;
  const g=new THREE.CylinderGeometry(0.013,0.013,1,5,1,true); g.rotateX(Math.PI/2); g.translate(0,0,-0.5);   // eje -Z, origen en la boca
...
const GUN_MUZZLE=new THREE.Vector3(0,0.014,-0.700);       // boca del caño (local)
const GUN_GRIP_R=new THREE.Vector3(0.012,-0.072,0.130);   // empuñadura → mano DERECHA
const GUN_GRIP_L=new THREE.Vector3(-0.064,-0.062,-0.105); // sobre el cargador, del lado de la cámara → mano IZQUIERDA (visible)
function buildGun(){ if(gunRoot)return;
  ...
  box(0.072,0.112,0.40, 0,0,-0.02, negro);          // cajón de mecanismos
  ...
  box(0.062,0.052,0.24, 0,0.004,-0.340, gris);      // guardamanos
  const ba=new THREE.Mesh(new THREE.CylinderGeometry(0.0115,0.0115,0.19,10), metal); ba.rotation.x=Math.PI/2; ba.position.set(0,0.014,-0.545); gunRoot.add(ba);   // caño
  const fh=new THREE.Mesh(new THREE.CylinderGeometry(0.019,0.019,0.05,10), negro); fh.rotation.x=Math.PI/2; fh.position.set(0,0.014,-0.665); gunRoot.add(fh);    // bocacha
  ...
  box(0.026,0.046,0.018, 0,0.089,-0.44, negro);          // punto de mira
  ...
  box(0.058,0.010,0.16, 0,-0.062,-0.30, negro);          // riel inferior del guardamanos
...
function tiroShoot(){
  ...
  if(gunRoot){ gunRoot.updateMatrixWorld(true); _mz.copy(GUN_MUZZLE).applyMatrix4(gunRoot.matrixWorld);
    const tr=tiroTracer(); tr.position.copy(_mz); tr.lookAt(end); tr.scale.set(1,1,Math.max(0.5,_mz.distanceTo(end))); tr.userData.v=0.075; tr.visible=true; tr.material.opacity=0.9; }
...
let mskBind=null, mskHips=null, mskSpine=null;
// Casi rígido a propósito: el clip de deslizada es de costado (por eso el juego lo espejaba), así que lo
// que se hereda de la cadera es un ALABEO, y con 17° ya alcanzaba para llevarse el hombro izquierdo contra
// la lente. La inclinación que se ve en la deslizada la pone la pose procedural, no el clip.
const MSK_CINT=0.12;   // tope de flexión de cintura respecto a la pose base (rad ≈ 7°)
function mskCapturar(raiz,clips){  // raiz = el rig LOCAL del jugador (nunca mpBodySrc ni un avatar remoto)
  ...
  raiz.traverse(o=>{               // pre-orden: padres antes que hijos
    ...
    porNom.set(o.name,r);
    if(enMask){ lst.push(r); if(n==='spine') mskSpine=r; } else hips=r;   // la cadera NO se restaura: es de las piernas
  });
  mskHips=hips;
...
/* TORSO ABSOLUTO: la cadera sigue 100% animada (las piernas la necesitan y el slide tiene que bajar de
   verdad), pero el tronco NO hereda su vuelco. Devolver los locales a la pose base no alcanza: el torso
   cuelga de Hips, así que al acostarse la cadera el torso rígido se iba de costado y los hombros
   terminaban al lado de la lente (medido: hombro izquierdo 6 cm ARRIBA del ojo al deslizarse).
   Fórmula: spineLocal = inv(caderaAhora) · caderaBase · spineBase → el tronco queda donde estaría si la
   cadera nunca se hubiera movido. Con tope de flexión, que además regala un lean natural en la deslizada. */
function mskTorso(){
  if(!mskHips||!mskSpine)return;
  _qa.copy(mskHips.h.quaternion).invert().multiply(mskHips.q).multiply(mskSpine.q);
  _qb.copy(mskSpine.q);
  const d=Math.min(1,Math.abs(_qb.dot(_qa))), ang=2*Math.acos(d);
  if(ang>MSK_CINT){ _qb.slerp(_qa, MSK_CINT/ang); mskSpine.h.quaternion.copy(_qb); }
  else mskSpine.h.quaternion.copy(_qa);
}
...
function ikTwoBone(up,low,end,target,pole,w){
  ...
  _aimBone(up,_qb,w,1.45);   // el brazo no se aparta más de ~83° de la animación
  ...
  _aimBone(low,_qb,w,2.0);   // el antebrazo tiene más margen
}
...
let TIRO_TORS_R=0.55, TIRO_TORS_L=-0.35;
...
const TIRO_VEN=[-0.08,0.34, -0.46,-0.07, 0.30,0.66];
...
const GUN_OJO=new THREE.Vector3(0.170,-0.215,0.335);
...
function _limitarArma(){
  ...
  for(let it=0;it<3;it++){
    ...
    bLArm.getWorldPosition(_gA); _gp2.copy(GUN_GRIP_L).applyMatrix4(gunRoot.matrixWorld);
    f=_gB.subVectors(_gp2,_gA).dot(_cf);
    if(f<0.14){ gunRoot.position.addScaledVector(_cf,0.14-f); gunRoot.updateMatrixWorld(true); mov=true; }
    // 2) dentro del alcance (se acerca el ARMA, nunca se estira el brazo)
    bRArm.getWorldPosition(_gA); _gp.copy(GUN_GRIP_R).applyMatrix4(gunRoot.matrixWorld);
    let d=_gA.distanceTo(_gp), al=_alcR*0.92;
    ...
    bLArm.getWorldPosition(_gA); _gp2.copy(GUN_GRIP_L).applyMatrix4(gunRoot.matrixWorld);
    d=_gA.distanceTo(_gp2); al=_alcL*0.92;
...
function tiroPostura(dt){
  ...
  mskTorso();         // y el tronco se desacopla del vuelco de la cadera (con tope de cintura)
  ...
  aSld+=((player.sliding?1:0)-aSld)*Math.min(1,12*dt);
  ...
  if(DBG.col){
    // 1) el torso acompaña el pitch de puntería, repartido en tres vértebras (nadie apunta al cielo sólo con los brazos)
    _spineAdd(bSp,  -pit*0.13,0,0, 1); _spineAdd(bSp01,-pit*0.17,0,0, 1); _spineAdd(bSp02,-pit*0.19,0,0, 1);
    // 2) pose: al esprintar el tronco se va adelante; al deslizarse se acuesta un poco y rola hacia el arma.
    //    (con la máscara el clip de deslizada ya no acuesta el torso: la inclinación la pone acá, medida)
    const inc=aSpr*0.10 + aSld*0.30, rol=aSld*0.08;
    _spineAdd(bSp,  inc*0.40,0,rol*0.40, 1); _spineAdd(bSp01, inc*0.30,0,rol*0.30, 1); _spineAdd(bSp02, inc*0.30,0,rol*0.30, 1);
  ...
  bSp02.getWorldPosition(_pecho);
  if(!_pechoOk){ _pechoS.copy(_pecho); _pechoOk=true; }
  else _pechoS.lerp(_pecho, Math.min(1,4*dt));
  _vida.subVectors(_pecho,_pechoS).multiplyScalar(TIRO_VIDA);
  ...
  const pPit = pit + lagP*0.85 + Math.sin(paso*2.0+0.7)*0.013*mov + jit + recUp*0.9 + landK*1.6
             + aSpr*0.22 + aSld*0.11 + aAir*0.10 - aRel*0.30;
  const pYaw = player.yaw - lagY*0.95 + jit*0.7 + aSpr*0.20 + aSld*0.05 - aRel*0.22;
  const pRol = -lagY*1.5 - latV*0.022 + Math.sin(paso)*0.020*mov + aSpr*0.16 + aSld*0.09 + aRel*0.30;
  ...
  gunRoot.position.copy(_ojo).add(_vida)
    .addScaledVector(_cr, GUN_OJO.x + bobX + respX + aSpr*0.028 + aSld*0.075)
    // al deslizarse el arma BAJA 12 cm: no es estética, es lo que mantiene el brazo izquierdo por debajo
    // de la lente (si el fusil queda a la altura de siempre, el antebrazo cruza el recorte y se ve en láminas)
    .addScaledVector(_cu, GUN_OJO.y + bobY + respY - landK*0.55 + aSpr*(-0.030) + aSld*(-0.120) - aRel*0.075)
    .addScaledVector(_cf, GUN_OJO.z + bobZ - recBack + aSpr*(-0.055) + aSld*(-0.030) - aRel*0.03);
  ...
  _gp2.copy(GUN_GRIP_L).applyMatrix4(gunRoot.matrixWorld);
  ...
  _gp.copy(GUN_GRIP_R).applyMatrix4(gunRoot.matrixWorld);
  gunRoot.position.add(_gB.subVectors(_hR,_gp)); gunRoot.updateMatrixWorld(true);   // el mango va A la mano derecha
  _gp2.copy(GUN_GRIP_L).applyMatrix4(gunRoot.matrixWorld);
  ...
  if(_gA.lengthSq()>1e-6 && _gB.lengthSq()>1e-6){ _gA.normalize(); _gB.normalize();
    ...
    if(ang>1e-4){ _ikN.crossVectors(_gA,_gB);
      if(_ikN.lengthSq()>1e-9){ _ikN.normalize(); _qa.setFromAxisAngle(_ikN,Math.min(ang,0.55));   // giro limitado
        gunRoot.quaternion.premultiply(_qa); gunRoot.updateMatrixWorld(true);
        _gp.copy(GUN_GRIP_R).applyMatrix4(gunRoot.matrixWorld);
        gunRoot.position.add(_gB.subVectors(_hR,_gp)); gunRoot.updateMatrixWorld(true); } } }
  TIRO_DEDOS_R.copy(_cf).multiplyScalar(0.62).addScaledVector(_cu,-0.72).addScaledVector(_cr,-0.30).normalize();
  TIRO_DEDOS_L.copy(_cr).multiplyScalar(0.86).addScaledVector(_cu,0.16).addScaledVector(_cf,0.46).normalize();
...
function tiroWire(){ if(_tiroWired)return; _tiroWired=true;
  ...
  addEventListener('mousemove',e=>{ if(document.pointerLockElement!==cv)return;
    ...
    player.pitch=Math.max(-0.9,Math.min(1.3,player.pitch-(e.movementY||0)*lk)); });
...
try{ window.__tiro={
  ...
  tors:(a,b)=>{ TIRO_TORS_R=a; TIRO_TORS_L=b; }, fogonazo:t=>{ flashT=(t==null?0.05:t); },
  ...
  manos:()=>{ if(!bRHand||!gunRoot)return null; const A=new THREE.Vector3(),B=new THREE.Vector3();
    ...
    const eR=+A.distanceTo(B).toFixed(3); bLHand.getWorldPosition(A); B.copy(GUN_GRIP_L).applyMatrix4(gunRoot.matrixWorld);
  ...
  esq:()=>{ const q=camera.quaternion.clone().invert(), P=new THREE.Vector3(), o={};
    ...
    g('cabeza',bodyHead); g('pecho',bSp02); g('homD',bRArm); g('homI',bLArm); g('manoD',bRHand); g('manoI',bLHand); g('cadera',bodyHips);
