...
const GUN_GRIP_R=new THREE.Vector3(0.012,-0.045,0.135);   // empuñadura → mano DERECHA. y = borde SUPERIOR de
...
const GUN_GRIP_L_ALTO=new THREE.Vector3(-0.052,-0.050,0.130);
const _gL=new THREE.Vector3(-0.040,-0.062,-0.060);        // agarre izquierdo VIVO (el que usa todo el código)
const GUN_GRIP_L=new THREE.Vector3(-0.040,-0.062,-0.060); // GUARDAMANOS (la palma cae en local −0,141, dentro
// del tramo −0,053…−0,293 de la caja nueva). x adentro de la huella del cajón para que el arma le corte la
// silueta a la manopla: sin huesos de dedos, que el arma TAPE la mano es lo que la hace leer como agarre.
function buildGun(){ if(gunRoot)return;
  ...
  box(0.062,0.052,0.24, 0,0.004,-0.173, gris);      // guardamanos (corrido 0.167 hacia atrás: ver GUN_GRIP_L)
  ...
  box(0.058,0.010,0.16, 0,-0.062,-0.133, negro);         // riel inferior del guardamanos
...
const GUN_OJO=new THREE.Vector3(0.110,-0.115,0.420);
...
function tiroPostura(dt){
  ...
  const pPit = pit + lagP*0.85 + Math.sin(paso*2.0+0.7)*0.013*mov + jit + recUp*0.9 + landK*1.6
             + aSpr*0.10 + sSld*0.08 + aAir*0.10 - aRel*0.30;   // 0.22 de esprint subía el arma 0,093 m
  const pYaw = player.yaw - lagY*0.95 + jit*0.7 + aSpr*SPR_YAW + sSld*0.04 - aRel*0.22;
  ...
  const pRol = -lagY*1.5 - latV*0.022 + Math.sin(paso)*0.020*mov + aSpr*0.16 + sSld*0.10*(player.slideRight?-1:1) + aRel*0.30;
  ...
  gunRoot.position.copy(_ojo).add(_vida)
    ...
    .addScaledVector(_cu, GUN_OJO.y + bobY + respY + 0.062*pUp - landK*0.55 + aSpr*(-0.085) + sSld*(-0.030) - aRel*0.075)
  ...
  if(_gA.lengthSq()>1e-6 && _gB.lengthSq()>1e-6){ _gA.normalize(); _gB.normalize();
    ...
    if(ang>1e-4){ _ikN.crossVectors(_gA,_gB);
      if(_ikN.lengthSq()>1e-9){ _ikN.normalize(); _qa.setFromAxisAngle(_ikN,Math.min(ang,0.55));   // giro limitado
        ...
        _gp.copy(GUN_GRIP_R).applyMatrix4(gunRoot.matrixWorld);
        _gB.subVectors(_hR,_gp); { const _L=_gB.length(); if(_L>GUN_CORREA) _gB.multiplyScalar(GUN_CORREA/_L); }
        gunRoot.position.add(_gB); gunRoot.updateMatrixWorld(true); } } }
