/* ============================================================================
   ...
   Lo que hay que saber antes de tocar este archivo (todo esto se midió):
     ...
     · REBOTE por restitución (−bvy*.55 con techo), no impulso fijo. Con el
       impulso fijo de 8,4 la pelota subía 2,07 u > RH (1,6): saltaba por encima
       del anillo anterior y quedaba rebotando sin avanzar (medido: 0 anillos en
       7 s en 900x430).
     ...
     · La cámara es un resorte con ADELANTO: mira más abajo cuanto más rápido
       cae, así el hueco de abajo siempre está en cuadro.
     · El RADAR 2D de abajo a la izquierda dibuja el próximo anillo desplegado:
       es lo que permite jugar sin adivinar cuando el anillo queda tapado.
     · Los tramos rojos se leen por TRES cosas a la vez: textura de rayas de
       peligro, contorno oscuro (casco invertido con side:BackSide) y pulso de
       emisión. Sólo el color no alcanzaba en pantalla chica.
     · El nivel 1 NO tiene rojo y el hueco del primer anillo sale casi al frente:
       nadie pierde antes de entender el juego.
   ========================================================================== */
const G={
  ...
  /* Los tres efectos propios entran con nombres que YA existen en el respaldo
     sintetizado del motor (wood = golpe corto, glass = quiebre agudo): si el mp3
     no se decodifica, el blip que suena sigue siendo el correcto. */
  sfx:{tap:A('sfx-tap.mp3'),pop:A('sfx-pop.mp3'),click:A('sfx-click.mp3'),coin:A('sfx-coin.mp3'),
       ...
       wood:A('sfx-helice-bounce.mp3'),glass:A('sfx-helice-crack.mp3'),swipe:A('sfx-helice-pass.mp3')},
...
const RIN=1.15,ROUT=2.55,RH=1.6,TH=.34;   /* radio interno/externo, paso, espesor */
...
const BHALF=.17;
const VMAX=14;                            /* velocidad terminal. A 21 la cadena de
...
let partK=1,fogK=1,decoK=1,shadowsOn=0;
const MAT={};let HAZ=null,OUTM=null,HTEX=null;
...
function hazTex(){
  ...
  t.wrapS=t.wrapT=T3.RepeatWrapping;t.repeat.set(5,5);
  if(T3.SRGBColorSpace)t.colorSpace=T3.SRGBColorSpace;
  return HTEX=t;
}
function hazMat(){
  if(HAZ)return HAZ;
  HAZ=new T3.MeshLambertMaterial({map:hazTex(),emissive:new T3.Color('#ff2a48'),emissiveIntensity:.1});
  return HAZ;
}
...
function diff(n){
  ...
  return {n,
    ...
    gap:Math.max(.80,1.52-n*.062),              /* hueco en radianes */
    redP:n<2?0:Math.min(.44,(n-1)*.045),        /* nivel 1 sin rojo */
...
function addSeg(R,a0,len,red){
  const o=new T3.Mesh(new T3.CylinderGeometry(ROUT,ROUT,TH,24,1,false,a0,len),
                      red?hazMat():mat(R.i%2?'#b9a1ff':'#a487f5'));
  R.grp.add(o);
  const inn=new T3.Mesh(new T3.CylinderGeometry(RIN,RIN,TH+.02,16,1,true,a0,len),
                        mat(red?'#7c0d20':'#d9dfec'));
  R.grp.add(inn);
  let out=null;
  if(red){
    out=new T3.Mesh(new T3.CylinderGeometry(ROUT+.08,ROUT+.08,TH+.1,24,1,false,a0-.025,len+.05),outMat());
    R.grp.add(out);
  }
...
function buildLevel(n){
  ...
  for(let i=0;i<nr;i++){
    ...
    /* el hueco del primer anillo sale casi al frente: margen de reacción */
    const g1=i===0?rnd(-.35,.35):rnd(0,TAU);
    ...
    const cuts=holes.map(h=>({a:norm(h[0])+Math.PI,l:h[1]})).sort((a,b)=>a.a-b.a);
    const spans=[];
...
function buildDeco(nr){
  ...
  const N=26;
  for(let i=0;i<N;i++){
    const a=rnd(0,TAU),r=rnd(5.4,10.5),y=-rnd(0,nr*RH+4);
    const s=rnd(.3,.95);
    const o=new T3.Mesh(new T3.BoxGeometry(s,s*rnd(.7,2.2),s),
      mat(pick(['#cbb7ff','#a9c4f2','#e6ecf7','#f3c9a0'])));
...
/* la pelota: GLB si llegó (facetada, se le ve girar), esfera si no */
function ballNode(){
  ...
  if(S&&S.scene){
    try{
      ...
      const g=new T3.Group();g.add(o);g.userData.glb=1;
      return g;
    }catch(e){console.warn('pelota glb',e);}
  }
  const m=new T3.Mesh(new T3.IcosahedronGeometry(BR,1),mat('#ff7a3d'));
  const band=new T3.Mesh(new T3.TorusGeometry(BR*.97,BR*.17,8,14),mat('#fff4e6'));
  band.rotation.x=Math.PI/2;
  const g=new T3.Group();g.add(m);g.add(band);
  return g;
}
G.resize=function(){
...
G.gfxApply=function(p){
  partK=p.part;fogK=p.fog;decoK=p.part;shadowsOn=p.sh;
  if(scene){
...
function placeCam(dt){
  const tgt=by+1.55+clamp(bvy,-VMAX,0)*.085;
...
function aimCam(y){
  cam.position.set(0,y,7.9);
  cam.lookAt(0,y-2.55,0);
}
...
G.step=function(dt){
  ...
  if(!dead&&!won){
    ...
    for(const R of rings){
      ...
      if(!s){                                     /* pasó por el hueco */
        ...
        const sp=ballScreen();
        ARC.sfx('swipe',{vol:.35,rate:clamp(1+combo*.04,1,1.7)});
        ARC.sfx('pop',{vol:.55,rate:clamp(1+combo*.055,1,2)});
        if(partK>.4)ARC.fx.ring(sp.x,sp.y,{r:70*(.6+partK*.5),life:.3,color:'#c9a7ff',w:4});
        if(combo>1)ARC.fx.text(sp.x,sp.y-ARC.H*.09,'x'+combo,{color:'#8b5cf6',size:Math.max(16,ARC.H*.055),life:.55});
      ...
      const sp=ballScreen();
      ARC.sfx('wood',{vol:.62,rate:clamp(.86+imp*.022,.85,1.45)});
      ARC.vib(8);
  ...
  /* pulso del rojo */
  if(HAZ)HAZ.emissiveIntensity=.10+.10*Math.sin(ARC.t*5.2);
  ...
  if(dead&&ARC.t-endT>.95){
    ...
    won=false;
    const st3=best>=total*.7?3:(best>=total*.4?2:1);
    ARC.over({win:true,score:passed,stars:st3,coins:15+lvl*5+best*2,
      sub:T('sRings')+': '+passed+'/'+total+'<br>'+T('sStreak')+': '+best+' · '+T('sBounce')+': '+bounces});
  }
};
...
/* El radar despliega el anillo que viene: el centro es el FRENTE (donde cae la
...
function drawRadar(g){
  ...
  g.beginPath();g.roundRect?g.roundRect(bx,byy,bw,bh,bh/2):g.rect(bx,byy,bw,bh);g.fill();
  if(R){
    const put=(w0,len,col)=>{
      ...
      while(l>0){
        ...
        g.fillStyle=col;g.fillRect(x0,byy+2,Math.max(1.5,x1-x0),bh-4);
        l-=seg;a=-Math.PI;
      }
    };
    for(const s of R.segs){
      if(s.len<=0)continue;
      put(s.a0+rot+R.off,s.len,s.red?'#ff3b57':'rgba(190,170,255,.92)');
    }
    if(R.spin){g.fillStyle='#f6d76b';g.font='900 '+Math.max(8,H*.022)+'px system-ui,sans-serif';
      g.fillText('↻',bx+bw+4,byy+bh-1);}
  }
  /* marca del frente = la pelota */
  const cx=bx+bw/2;
  g.fillStyle='#ff7a3d';
  g.fillRect(cx-1.5,byy-3,3,bh+6);
  g.fillStyle='rgba(255,255,255,.75)';
  g.font='700 '+Math.max(8,H*.021)+'px system-ui,sans-serif';
  g.fillText(T('next'),bx,byy-Math.max(4,H*.012));
}

/* ------------------------------------------------------------------- SONDA */
G.dbg={
  state:()=>({passed,total,dead,won,by:+by.toFixed(2),vy:+bvy.toFixed(1),lvl,combo,best,bounces,
    rot:+rot.toFixed(2),cam:+camY.toFixed(2),
  ...
  autoMove:()=>{
    ...
    const tt=(bvy+Math.sqrt(bvy*bvy+2*grav*d))/grav;
    ...
    const pen=c=>{
      if(!R2)return 0;
      const s2=segAt(R2,norm(c+offAt-(R2.off+R2.spin*(tt+RH/Math.max(2,-bvy+3)))));
      return s2&&s2.red?5:0;
    };
    ...
    const vHit=Math.sqrt(Math.max(0,bvy*bvy+2*grav*d));
