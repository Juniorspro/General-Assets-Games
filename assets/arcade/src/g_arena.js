...
const G={
  ...
  glb:{heroe:A('m-arena-heroe.glb'),jefe:A('m-arena-jefe.glb')},
  i18n:{
...
let joy=null,botOn=0,demo=0,tut=0;
...
function buildArena(){
  ...
  for(let ix=-RW+TS*2;ix<RW-.1;ix+=TS*2)vplate(ac,ix,0,.07,RH*2,.006,CC('#2b2350'));
  for(let iz=-RH+TS*2;iz<RH-.1;iz+=TS*2)vplate(ac,0,iz,RW*2,.07,.006,CC('#2b2350'));
  ...
  vbox(ac,0,0,-RH-.02,3.3,2.15,.12,CC('#0b0716'),CC('#0b0716'));
  ...
  vbox(ac,0,-1.25,RH+.6,RW*2+1.4,1.25,.5,CC('#1a1230'),CC('#120c22'));
  vbox(ac,-RW-.6,-1.25,0,.5,1.25,RH*2+1.4,CC('#1a1230'),CC('#120c22'));
  vbox(ac, RW+.6,-1.25,0,.5,1.25,RH*2+1.4,CC('#1a1230'),CC('#120c22'));
  /* braseros en las cuatro esquinas */
  for(const sx of [-1,1])for(const sz of [-1,1]){
    vbox(ac,sx*(RW-.42),0,sz*(RH-.42),.3,1.15,.3,CC('#6a58a8'),CC('#33265c'));
...
function buildObs(){
  ...
  const ac=VA(),ct=CC('#8f79d4'),cs=CC('#4a3a78');
  for(const o of obs){vbox(ac,o.x,0,o.z,o.r*1.95,o.h,o.r*1.95,ct,cs);
    vbox(ac,o.x,o.h,o.z,o.r*2.2,.14,o.r*2.2,CC('#b7a2f2'),CC('#6a58a8'));}
...
function buildPools(){
  ...
  /* tres siluetas de enemigo, tres mallas instanciadas, 3 llamadas en total */
  IE=[
    IM(new T3.IcosahedronGeometry(.5,0),lam('#ffffff'),CAPE),   /* mole / divisor */
    IM(new T3.ConeGeometry(.5,1.15,6),lam('#ffffff'),CAPE),     /* torre */
    IM(new T3.OctahedronGeometry(.56,0),lam('#ffffff'),CAPE)    /* púa */
  ];
...
function glbNode(key,targetH,yaw){
...
function mkEnemy(t,pl,x,z,gen){
  ...
  const B={mole :{shape:0,hp:9 ,sp:1.9,r:.5 ,dmg:1,col:PAL.e1,sc:1  },
           ...
           div  :{shape:0,hp:16,sp:1.4,r:.72,dmg:1,col:PAL.e4,sc:1.4},
           mini :{shape:0,hp:6 ,sp:2.6,r:.36,dmg:1,col:PAL.e4,sc:.7 }}[t];
  const yo=B.shape===1?.6:(B.shape===2?.56:.5);
...
function fire(){
  ...
  ARC.sfx('shoot',{vol:.5,rate:1+rnd(-.06,.06)});
...
G.step=function(dt){
  ...
  if(jv){
    ...
    vx=jv.x*P.spd;vz=jv.y*P.spd;
    moving=1;stillT=0;
    aim=Math.atan2(vx,vz);
  }else{vx=vz=0;stillT+=dt;}
  ...
  /* DISPARO AUTOMÁTICO: sólo quieto. Es la mecánica del género. */
  shotT-=dt;
  if(!moving&&stillT>.08){
    const tg=nearestEnemy(hx,hz);
    if(tg){
      aim=Math.atan2(tg.x-hx,tg.z-hz);
      if(shotT<=0)fire();
    }
  }
  stepEnemies(dt);
...
function drawScene(a){
  ...
  if(hn){
    ...
    /* rebote al caminar y parpadeo de invulnerabilidad */
    const bounce=moving?Math.abs(Math.sin(ARC.t*11))*.09:0;
    hn.position.y+=bounce;
